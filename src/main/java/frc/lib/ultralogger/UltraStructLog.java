...
public class UltraStructLog<T> implements UltraLogEntry<T> {
  ...
  private void checkDLFMS() {
    ...
    this.datalogPublisher =
        Optional.of(StructLogEntry.create(DataLogManager.getLog(), logName, struct));
  }

  private void checkNTFMS(boolean doTimestamp) {
    if (doTimestamp
        && System.currentTimeMillis() - lastCheckedTimestamp < TelemetryConstants.fmsCheckDelay) {
      return;
    }
    ...
    if (this.ntPublisher.isEmpty()) {
      this.ntPublisher =
          Optional.of(NetworkTableInstance.getDefault().getStructTopic(logName, struct).publish());
    }
  }

  public void update(T item) {
    if (TelemetryConstants.killswitch || errored) {
      return;
    }
    try {
      if (item == null) {
