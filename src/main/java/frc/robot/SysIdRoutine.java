...
public class SysIdRoutine extends edu.wpi.first.wpilibj2.command.sysid.SysIdRoutine {
    ...
    public SysIdRoutine(Config config, Mechanism mechanism, Subsystem subsystem, BooleanSupplier forwardLimitSupplier, BooleanSupplier backwardLimitSupplier) {
        super(config, mechanism);
    ...
    public Command quasistatic(Direction direction) {
        Command command = super.quasistatic(direction).until(limitSupplier(direction)).withName("SysID Quasistatic: " + direction.toString());
    ...
    public Command dynamic(Direction direction) {
        Command command = super.dynamic(direction).until(limitSupplier(direction)).withName("SysID Dynamic: " + direction.toString());
        command.addRequirements(subsystem);
        return command;
    }
}
