...
public final class PhotonlibCamera implements Camera {
  ...
  public PhotonlibCamera(String name, Transform3d offset, AprilTagFieldLayout layout) {
    ...
    m_Estimator =
        new PhotonPoseEstimator(layout, PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR, offset);
  }
  ...
  private void update(Result result, PhotonPipelineResult cameraResult) {
    Optional<EstimatedRobotPose> estimatedPose =
        isResultValid(cameraResult) ? m_Estimator.update(cameraResult) : Optional.empty();
    if (!estimatedPose.isPresent()) {
      return;
    }

    var pose = estimatedPose.get();
    if (m_LastTimestamp.isPresent()
        && pose.timestampSeconds - m_LastTimestamp.get() > Double.MIN_VALUE) {
      return;
    }
